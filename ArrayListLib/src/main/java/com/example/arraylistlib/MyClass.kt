...
//            println("${Test.count}")
            /**
             * 创建型  单例 原型 构建者 工厂 抽象工厂
             * 行为型  责任链 中介 观察者 策略 命令 备忘录 迭代器 访问者 模板方法 解释器
             * 结构型  代理 享元 组合 适配器 外观 桥接 装饰者
             *
             * 六大设计原则 ：
             * 1. 单一职责
             * 2. 接口隔离
             ...
             * 7. 合成复用
             *
             * 今天再度想起那事，每每都想不明白，当时是怎么想的，说什么信什么，当时的我是真的我吗?
             * 任何一个点的停顿都可以阻止我继续下去
             * 或者还是我太相信社会治安了?
             *
             *
             */
            println("args = [${System.currentTimeMillis() / 1000}]")
            val s = "43B08C50F56E4A252C6C336963D74975"
    ...
    fun shellSort() {
        ...
        var step = array.size / 2 // size = 8 4 2 1
        while (step > 0) {
            list.add(step)
            step /= 2
        }
    ...
    fun outChar(start: Char, end: Char, sb: String) {
        if (start > end) {
            print("$sb,,")
            ++Test.count
            return
        }
        outChar((start.toInt() + 1).toChar(), end, sb + start)
        outChar((start.toInt() + 1).toChar(), end, sb)
    }
